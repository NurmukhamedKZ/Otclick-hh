...
async def test_third_independent_transient_hh_failure_becomes_terminal_score_error():
    ...
    with (
        patch.object(svc.vacancy_pipeline, "claim_for_scoring", return_value="claim-3"),
        patch.object(svc.vacancy_pipeline, "release_scoring_claim", return_value=False),
        patch.object(svc.vacancy_pipeline, "transition", return_value=True) as transition,
        patch.object(
            svc.vacancy_review_service,
    ...
    assert final["expected_claim_token"] == "claim-3"
...
async def test_scoring_batch_opens_circuit_after_two_consecutive_hh_failures():
    ...
    rows = [{"id": f"p{i}", "score_attempts": 0} for i in range(5)]
