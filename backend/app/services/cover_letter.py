...
FALLBACK_TEMPLATE = (
    "{Здравствуйте|Добрый день}! Пишу по поводу вакансии "
    "«%(vacancy_name)s» в %(employer_name)s. {Прочитал описание|Изучил, что "
    "нужно делать} - {задача понятна и близка|похоже на мой опыт|походит на то, "
    "что я уже делал}: по резюме «%(resume_title)s» {у меня есть|накоплен} "
    "релевантный опыт. {Если по сути|Коротко}: {готов обсудить, чем могу быть "
    "полезен|хочу узнать больше о задаче и команде|расскажу подробнее на "
    "интервью}. {Когда удобно созвониться?|До связи.}"
)
